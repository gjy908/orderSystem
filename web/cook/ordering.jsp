...
<div class="container mx-auto px-4">
    <div class="header text-3xl font-bold text-center my-6">正在进行的订单列表</div>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <c:forEach var="orderDetails" items="${orderDetailsList}">
            <div class="bg-white p-6 rounded-lg shadow-md">
                <h3 class="text-xl font-bold mb-4">订单ID: ${orderDetails.order.orderId}</h3>
                <p>创建时间: ${orderDetails.order.createdAt}</p>
                <p>更新时间: ${orderDetails.order.updatedAt}</p>

                <div class="mt-4">
                    <h4 class="text-lg font-bold">订单项</h4>
                    <div class="space-y-4">
                        <c:forEach var="itemDetails" items="${orderDetails.orderItems}">
                            <div class="p-4 border rounded-lg <c:choose>
                                ...
                                <p class="description" onmouseover="showTooltip(this)" onmouseout="hideTooltip(this)"
                                   data-tooltip="${itemDetails.menu.description}">
                                ...
                                <p><strong>下单时间:</strong> ${itemDetails.orderItem.createdAt}</p>
                                <p>
                                    <strong>状态:</strong>
                                    <span class="<c:choose>
                                                    <c:when test='${itemDetails.orderItem.status == 0}'>status-waiting</c:when>
                                ...
                                <button class="bg-blue-500 text-white py-1 px-3 rounded hover:bg-blue-700"
                                        onclick="showStatusModal('${itemDetails.orderItem.orderItemId}')">切换状态
                                </button>
                            </div>
                        </c:forEach>
                    </div>
